...
function channeldelcon(chnum) {
	...
	function getParameterValues() {
		...
		return "?command=channelDelete&channel_seq=" + channel_seq
				+ "&member_id=" + member_id;
	}
...
function callChatList(chnum) {
	...
	$
			.ajax({
				url : "ChatController?command=callChatList&channel_seq="
						+ chnum,
				dataType : "json",
				method : "post",
...
function channelInfo(chnum) {
	...
	$.ajax({
		...
		success : function(data) {
			...
			span.innerHTML = " 채널번호 : " + channel_seq + "<br>" + " 채널정보 : "
					+ channel_info + "<br>" + " 채널생성일 : " + channel_regdate;
...
function callMessageList(msgRoomNum) {
	...
	$
			.ajax({
				url : "ChatController?command=callMessageList&messageroom_seq="
						+ msgRoomNum,
				dataType : "json",
				method : "post",
				success : function(data) {
...
function messsageInfo(msgRoomNum) {
	...
	$.ajax({
		url : "ChatController?command=msgRoomSelect&messageroom_seq="
				+ msgRoomNum,
		dataType : "text",
		method : "post",
		success : function(data) {
			...
			span.innerHTML = " 워크스페이스 : " + workspace_seq + "번의 " + member_name
...
function callInviteMessageMemberList() {
	...
	function getParameterValues() {
		return "?command=callInviteMessageMemberList&workspace_seq=" + workspace_seq
	...
	$.ajax({
		...
		success:function(data) {
			...
			var ul = document.createElement('ul');
			ul.setAttribute("class", "list-group");
			document.getElementById('messageInviteMemberList').appendChild(ul);
			...
			for (var i = 0; i < list[0].length; i++) {
				...
				document.getElementById('messageInviteMemberList').appendChild(li);
			}
		},
		error:function(){
			alert("통신 실패")
...
function createMessageRoom() {
	...
	chks.forEach((node) => {
		    if(node.checked)  {
		    	...
		    	function getParameterValues() {    		
		    		return "?command=createMessageRoom&workspace_seq=" + workspace_seq + "&member_id=" + member_id + "&member_name=" + member_name + "&member2_id=" + member2_id + "&member2_name=" + member2_name;
	...
	if (content.value != "") {
		...
		if (member2_id != null) {
			...
			function getParameterValues() {
				...
				return "?command=messageInsert&messageroom_seq="
						+ messageroom_seq + "&to_id=" + to_id + "&to_name="
						+ to_name + "&from_id=" + from_id + "&from_name="
						+ from_name + "&message_content=" + message_content;
			}
			...
			function getParameterValues() {
				...
				return "?command=chatInsert&channel_seq=" + channel_seq
						+ "&member_id=" + member_id + "&member_name="
						+ member_name + "&chat_content=" + chat_content;
			}
...
function workspaceDelcon(wsseq) {
	...
	if (con) {
		location.href = 'ChannelController?command=WorkSpaceDel&workspace_seq='
				+ workspace_seq;
	}
}
...
function callWorkspaceMemberList() {
	...
	function getParameterValues() {
		...
		return "?command=callWorkspaceMemberList&workspace_seq="
				+ workspace_seq + "&member_id=" + member_id;
	}
...
function banishWorkspace() {
	...
	if (con) {
		...
		var chks = document.getElementsByName("banishChk");
		var workspace_seq = $("#workspace_seq").val();
		var workspace_name = $("#workspace_name").val();
...
function callWorkspaceInviteList() {
	...
	function getParameterValues() {
		...
		return "?command=callWorkspaceInviteList&member_id=" + member_id
				+ "&workspace_seq=" + workspace_seq;
	}
	...
	$.ajax({
		...
		success : function(data) {
			...
			label.setAttribute("class", "control-label");
			label.innerHTML = "초대 가능 리스트"
			document.getElementById('workspaceInviteList').appendChild(label);

...
<c:catch var="err">
	...
	<rss><x:forEach select="$wrss/response/body/items/item" var="coco"><gubun><x:out select="$coco/gubun"/></gubun><defCnt><x:out select="$coco/defCnt"/></defCnt><incDec><x:out select="$coco/incDec"/></incDec></x:forEach></rss>
</c:catch>
<c:if test="${err!=null}">
	${err}
</c:if>
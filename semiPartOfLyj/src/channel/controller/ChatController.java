...
public class ChatController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			if (command.equals("channeladd")) {
			...
			} else if (command.equals("addMemberList")) {

				String channel_name = request.getParameter("channel_name");
				List<RoomMemberDto> list = biz.otherMemeberList(channel_name);

				String res = "";
				
				for (int i = 0; i < list.size(); i++) {
					if (i == list.size() -1 ) {
					res += list.get(i).getMember_name() + "|\\|" + list.get(i).getMember_id();
					} else {
					res += list.get(i).getMember_name() + "|\\|" + list.get(i).getMember_id() + "|br|";	
					}
			...
			} else if (command.equals("delChannelMember")) {
				...
				int res = biz.delChannelMember(dto);
				if (res > 0) {
					String msg = "채널 맴버 삭제 성공";
					response.getWriter().append(msg);
				} else {
					String msg = "채널 맴버 삭제 실패";
					response.getWriter().append(msg);
				}
			} else if (command.equals("addChannelMember")) {
				...
				int res = biz.roomMemberAdd(dto);
				
				if (res > 0) {
					String msg = "채널 맴버 추가 성공";
					response.getWriter().append(msg);
				} else {
					String msg = "채널 맴버 추가 실패";
					response.getWriter().append(msg);
				}
			}
		} catch (Exception e) {
